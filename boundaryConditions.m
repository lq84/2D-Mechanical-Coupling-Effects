...
thetaMoves = zeros(nn,1); thetaMag = epsilon*L/2; %0.01 radians
F = zeros(nn*3,1);

uRot = zeros(nn,3);
tol = 10^-5*L; %Nodes that are this close to the boundary or closer 
...
%         thetaFixed = zeros(nn,1);
    ...
    case 'bendYfree'
        %This boundary condition is like bendYfree, but only the center
        %is fixed in the x and y direction.
        botNodes = and(globalPos(:,2) <= tol,globalPos(:,2) >= -tol);
        topNodes = and(globalPos(:,2) <= macroHeight+tol,...
                         globalPos(:,2) >= macroHeight-tol);
        centerNodes = and(and(globalPos(:,2) <= macroHeight/2+tol,globalPos(:,2) >= macroHeight/2-tol),...
            and(globalPos(:,1) <= macroWidth/2+tol ,globalPos(:,1) >= macroWidth/2-tol));
        xFixed = centerNodes;
        yFixed = centerNodes;
%         thetaFixed = centerNodes;
        for nodeI=find(topNodes)
            thetaMoves(nodeI) = 1;
        end
        for nodeI=find(botNodes)
            thetaMoves(nodeI) = -1;
        end
